...
//// cache sim
//////////////////////////////////////////////////////////////////////////////
FILE *Stramash_Print_Per_Sec_File_x86;
FILE *Stramash_Print_Per_Sec_File_arm;
...
#define L2_x86_overhead 14

#define L1_arm_overhead 4
#define L2_arm_overhead 9

#define L3_arm_overhead 50
#define L3_x86_overhead 70
...
#define STOP_SWITCH_ARM 0x1200000000
...
typedef struct
{
    ...
    u_int64_t debug_ringbuffer;
    u_int64_t debug_inaccessible1;
    u_int64_t debug_inaccessible2;
    u_int64_t debug_peer_cache;
...
#define _g_new(typ, nr) (is_stramash ? (typ *)new_shared(sizeof(typ) * (nr)) : (typ *)g_new(typ, nr))
#define new_L3(typ, nr) (is_stramash ? (typ *)new_shared_L3(sizeof(typ) * (nr)) : (typ *)g_new(typ, nr))
#define _g_new0(typ, nr) (is_stramash ? (typ *)new_shared(sizeof(typ) * (nr)) : (typ *)g_new0(typ, nr))
#define g_free(ptr) \
    do              \
    {               \
    } while (0)
...
static int in_cache_valid(Cache *cache, uint64_t addr)
{
    int i;
    uint64_t tag, set;
    ...
    for (i = 0; i < cache->assoc; i++)
    {
        if (cache->sets[set].blocks[i].tag == tag &&
            cache->sets[set].blocks[i].valid)
        {
            if(cache->sets[set].blocks[i].state == INVALID){
                return -1;
            }
...
static bool check_state(Cache *cache, uint64_t addr, CacheState state)
{
    int i;
    uint64_t tag, set;
    ...
    for (i = 0; i < cache->assoc; i++)
    {
        if (cache->sets[set].blocks[i].tag == tag &&
            cache->sets[set].blocks[i].valid)
        {
            if (cache->sets[set].blocks[i].state == state)
            {
                return true;
            }
        }
...
{
    // qemu_plugin_feedback(runtime);
    shmcatalog->running_info->runtime += runtime;
    shmcatalog->insn_count += runtime;
}

// Feedback overhead of memory access
static void mem_access_overhead(CacheLevel cache_level, bool isStore)
{
    ...
    case L3:
        ...
        add_runtime(prof_arch == PROF_ARCH_AARCH64 ? L3_arm_overhead : L3_x86_overhead);
        break;
...
static bool access_cache(Cache *cache, uint64_t addr, ReturnInfo *ret_info)
{
    ...
    ret_info->effective_addr = cache->sets[set].blocks[replaced_blk].effective_addr;
    ...
    cache->sets[set].blocks[replaced_blk].effective_addr = addr;

    ret_info->blocks = &cache->sets[set].blocks[replaced_blk];
    return false;
}

static uint64_t remove_offset(uint64_t addr)
{
    if (addr < 0x100000000 && addr > 0x40000000 && prof_arch == PROF_ARCH_AARCH64)
    {
...
static bool in_peer_cache(uint64_t effective_addr)
{
    ...
    result = result || !(in_cache(other_shmcatalog->l1_dcaches[0], effective_addr) == -1);
    // g_mutex_unlock(&other_shmcatalog->l2_ucache_locks[0]);
    shmcatalog->running_info->debug_peer_cache++;
    return result;
}
...
static bool is_peer_modified(uint64_t effective_addr)
{
    // if (mode != STRAMASH)
    // {
    //     return false;
    // }
    bool result = check_state(other_shmcatalog->l2_ucaches[0], effective_addr, MODIFIED);
    result = result || check_state(other_shmcatalog->l1_dcaches[0], effective_addr, MODIFIED);

    return result;
}

static void broadcast_peer_state(uint64_t effective_addr, CacheState state)
{
    // if(mode != STRAMASH){
    //     return;
    // }
    shmcatalog->running_info->broadcast++;
    add_runtime(70);
...
static void broadcast_state(Cache **cache_ptr, uint64_t effective_addr, CacheState state)
{
    find_and_set(cache_ptr, effective_addr, state);
    return;
}

static void write_info(RunningInfo *info, FILE *file)
{
    int id = 0;
    ...
    // Write numbers to the file, separated by a space, and end with a newline
    // flock(file, LOCK_EX);
...
{
    RunningInfo *info = shmcatalog->running_info;
    // write_info(info, Stramash_Print_Per_Sec_File_x86);
    ...
    {
        write_info(info, Stramash_Print_Per_Sec_File_arm);
    }
}
static void print_info(RunningInfo *info)
{
    ...
    printf("DEBUG peer cache: %lu\n", info->debug_peer_cache++);
    printf("DEBUG broadcast: %lu\n", info->broadcast);
    printf("DEBUG unaccess 1: %lu\n", info->debug_inaccessible1);
    printf("DEBUG unaccess 2: %lu\n", info->debug_inaccessible2);
    // printf("Inter-Processor Interrupts: %lu\n", info->ipi);
    printf("Number of Instructions: %lu\n", info->nr_of_inst);
    printf("Number of mem_access: %lu\n", info->nr_of_mem_access);
    printf("Runtime: %lu\n", info->runtime);
}
static void print_info2(RunningInfo *info1,RunningInfo *info2)
{
    printf("Runtime: %lu\n", info1->runtime + info2->runtime);
}
static void print_result()
{
    // print_info2(shmcatalog->running_info,other_shmcatalog->running_info);
    // return;
    ...
    {
        printf("Arm:\n");
        info = other_shmcatalog->running_info;
        print_info(info);
    }
}
...
static void clean_both_cache()
{
    ...
    memset(other_shmcatalog->running_info, 0, sizeof(RunningInfo));
}
...
static void is_local_addr(uint64_t effective_addr)
{
        if (test == OPENPITON_MEM)
        {
            mem_access_overhead(DRAM, false);
        }
        else
        {
            if (mode == STRAMASH)
            {
                if (test != SHARED_MEM)
                {
                    if (prof_arch == PROF_ARCH_AARCH64)
                    {
                        ...
                        {
                            mem_access_overhead(REMOTE, false);
                        }
                        else if (effective_addr >= 0x100000000 && effective_addr < 0x150000000)
                        {
                            mem_access_overhead(REMOTE, false);
                        }
                        else if (effective_addr >= 0x220000000)
                        {
                            mem_access_overhead(REMOTE, false);
                        }
                        else
                    ...
                    else
                    {
                        ...
                        else if (effective_addr >= 0x150000000 && effective_addr < 0x220000000)
                        {
                            mem_access_overhead(REMOTE, false);
                        }
                        else
                ...
                else
                {
                    if (prof_arch == PROF_ARCH_AARCH64)
                    {
                        ...
                        else if (effective_addr >= 0x100000000 && effective_addr < 0x200000000)
                        {
                            mem_access_overhead(REMOTE, false);
                        }
                        else if (effective_addr >= 0x220000000)
                    ...
                    else
                    {
                        ...
                        else if (effective_addr >= 0x100000000 && effective_addr < 0x220000000)
                        {
                            mem_access_overhead(REMOTE, false);
                        }
                        else
            ...
            else
            {
                ...
                {
                    // arm
                    // Ring buffer tong start
                    if (effective_addr >= 0x80000000 && effective_addr <= 0x84000000) // arm ring buffer
                    {
                        mem_access_overhead(REMOTE, false);
                    }
                    else if (effective_addr >= 0x90000000 && effective_addr <= 0x94000000) // arm ring buffer
                    {
                        mem_access_overhead(REMOTE, false);
                    }
                    ////////////Tong ring buffer done
                    else if (0xA0000000 >= effective_addr && effective_addr >= 0x40000000)
                    {
                        shmcatalog->running_info->debug_inaccessible1++;

                        mem_access_overhead(DRAM, false);
                    }
                    else if (effective_addr >= 0x100000000 && effective_addr < 0x210000000)
                    {
                        shmcatalog->running_info->debug_inaccessible2++;
                ...
                else
                {
                    // x86
                    // Ring buffer tong start
                    if (effective_addr >= 0x40000000 && effective_addr <= 0x44000000) // x86 ring buffer
                    {
                        mem_access_overhead(REMOTE, false);
                    }
                    else if (effective_addr >= 0x50000000 && effective_addr <= 0x54000000) // x86 ring buffer
                    {
                        mem_access_overhead(REMOTE, false);
                    }
                    // TONG RING BUFFER DONE
                    else if (0xC0000000 >= effective_addr && effective_addr >= 0x60000000)
                    {
                        shmcatalog->running_info->debug_inaccessible1++;
                        mem_access_overhead(DRAM, false);
                    }
                    else if (0x210000000 >= effective_addr && effective_addr >= 0x200000000)
                    {
                        shmcatalog->running_info->debug_inaccessible2++;
                        mem_access_overhead(DRAM, false);
                    }
                            ...
                            uint64_t vaddr, void *userdata)
{
    ...
    remove_offset(effective_addr);

    if (test == PRINT_DATA)
    {
        if ((effective_addr) == _switch)
        {
        ...
        {
            effective_addr = effective_addr >> 4;
            if (prof_arch == PROF_ARCH_X86_64)
            {
                fprintf(Stramash_Print_Per_Sec_File_x86, "%lx\n", effective_addr);
            }
            else
            {
                fprintf(Stramash_Print_Per_Sec_File_arm, "%lx\n", effective_addr);
            }
        }
        return;
    }
    ...
    {
        printf("own curr icount: %lx\n", shmcatalog->icount_base);
        printf("other curr icount: %lx\n", other_shmcatalog->icount_base);
        ...
        if (shmcatalog->start_sim == 0)
        {
            ...
            shmcatalog->icount_base = 0;
            other_shmcatalog->icount_base = 0;

            shmcatalog->insn_count = 0;
            shmcatalog->prev_icount = 0;

            other_shmcatalog->insn_count = 0;
            other_shmcatalog->prev_icount = 0;
    ...
    if (is_stramash && (effective_addr) == _stop_switch)
    {
        if (shmcatalog->start_record == 0)
        {
            ...
            other_shmcatalog->start_record = 1;
            return;
        }
        else
        {
            ...
            other_shmcatalog->start_record = 0;
    ...
    {
        // L1 hit
        mem_access_overhead(L1, false);
        if (is_store)
        {
            if (l1_ret.blocks->state == SHARED)
            {
                broadcast_peer_state(effective_addr, INVALID);
                broadcast_state(l2_ucaches[cache_idx], effective_addr, INVALID);
            }
    ...
    {
        if (l1_ret.blocks->state == MODIFIED)
        {
            mem_access_overhead(L2, true);
            broadcast_state(l2_ucaches[cache_idx], effective_addr, MODIFIED);
        }
    }
    ...
    {
        // L2 hit
        mem_access_overhead(L2, false);
        if (is_store)
        {
            if (l2_ret.blocks->state == SHARED)
            {
                broadcast_peer_state(effective_addr, INVALID);
                broadcast_state(l3_ucache, effective_addr, INVALID);
            }
    ...
    {
        if (l2_ret.blocks->state == MODIFIED)
        {
            mem_access_overhead(L3, true);
            broadcast_state(l3_ucache, effective_addr, MODIFIED);
        }
    }
    ...
    if (!hit_in_l3)
    {   
        if((shmcatalog->start_record == 1 || other_shmcatalog->start_record == 1)){
            // printf("L3 Load: %lx\n", effective_addr);
        }
        if (l3_ret.blocks->state == MODIFIED)
        {
            mem_access_overhead(DRAM, true);
        }
        ...
        {
            if(is_peer_modified(effective_addr)){

                mem_access_overhead(DRAM, false);
                mem_access_overhead(REMOTE, false);  

                broadcast_peer_state(effective_addr, SHARED);
                // peer cache hit
                l3_ret.blocks->state = SHARED;
        ...
        is_local_addr(effective_addr);
    }
    else
    {
        // L3 hit
        // sync the state of L1, L2 and L3
        // if (l3_ret.blocks->state == INVALID)
        // {
        //     mem_access_overhead(L3, false);
        // }
        // if (in_peer_cache(effective_addr))
        // {
        //     broadcast_peer_state(effective_addr, SHARED);
...
*/
        ...
        // if (shmcatalog->prev_icount == 0)
        // {
        //     shmcatalog->prev_icount = qemu_plugin_get_icount();
        // }

        // shmcatalog->icount_base += qemu_plugin_get_icount() - shmcatalog->prev_icount;
        // shmcatalog->insn_count += qemu_plugin_get_icount() - shmcatalog->prev_icount;
        // shmcatalog->prev_icount = qemu_plugin_get_icount();

        if (shmcatalog->insn_count > 1000)
        {
            shmcatalog->insn_count = shmcatalog->insn_count - 1000;

            if (prof_arch == PROF_ARCH_AARCH64 && shmcatalog->start_sim == 1)
            {
                sem_arm = (uint64_t)1;
                ...
                //     if (total_instruction_count >= 1000)
                //     {
                //         write_stats();
...
static void plugin_exit(qemu_plugin_id_t id, void *p)
{
    ...
    // if (prof_arch == PROF_ARCH_X86_64)
    // {
    //     fclose(Stramash_Print_Per_Sec_File_x86);
    // }
    // else
    // {
    //     fclose(Stramash_Print_Per_Sec_File_arm);
    // }
}
...
static void vcpu_mem_access_debug(uint64_t effective_addr, bool is_store)
{
    struct qemu_plugin_hwaddr *hwaddr;
    int cache_idx = 0;
    InsnData *insn;
                        ...
                        int argc, char **argv)
{
    ...
    for (i = 0; i < argc; i++)
    {
        ...
        {
            mode = STRTOLL(tokens[1]);
        }
        else if (g_strcmp0(tokens[0], "cache") == 0)
        {
            cache_factor = STRTOLL(tokens[1]);
        }
    ...
    // l1_dassoc = 8;
    // l1_dblksize = 64;
    // l1_dcachesize = l1_dblksize * l1_dassoc * 64;
    ...
    // l2_assoc = 8;
    // l2_blksize = 64;
    // l2_cachesize = l2_assoc * l2_blksize * 256;

    // l3_assoc = 20;
    // l3_blksize = 64;
    // l3_cachesize = l3_assoc * l3_blksize * 800 * cache_factor;
    ...
    l1_iassoc = 8;
    l1_iblksize = 64;
    l1_icachesize = l1_iblksize * l1_iassoc * 64;

    l2_assoc = 16;
    l2_blksize = 64;
    l2_cachesize = l2_assoc * l2_blksize * 1024;

    l3_assoc = 16;
    l3_blksize = 64;
    l3_cachesize = l3_assoc * l3_blksize * 1024 * 4;
    ...
    // l3_assoc = 16;
    // l3_blksize = 64;
    // l3_cachesize = l3_assoc * l3_blksize * 1024 * cache_factor;
    ...
    //
    // Write curr state in File
    //
    if (test == PRINT_DATA)
    {
        const char *base_filename_x86 = "Stramash_Print_MEM_ACCESS_File_x86";
        const char *base_filename_arm = "Stramash_Print_MEM_ACCESS_File_arm";
        char filename_x86[100];
        char filename_arm[100];
        snprintf(filename_x86, sizeof(filename_x86), "%s_Stramash_id_%d", base_filename_x86, Stramash_id);
        snprintf(filename_arm, sizeof(filename_arm), "%s_Stramash_id_%d", base_filename_arm, Stramash_id);
        if (prof_arch == PROF_ARCH_X86_64)
        {
            Stramash_Print_Per_Sec_File_x86 = fopen(filename_x86, "w");
        }
        else
        {
            Stramash_Print_Per_Sec_File_arm = fopen(filename_arm, "w");
        }
    }
    else if (shmcatalog->start_record == 1)
    {
        const char *base_filename_x86 = "Stramash_Print_Per_Sec_File_x86";
        const char *base_filename_arm = "Stramash_Print_Per_Sec_File_arm";
    ...
    // DEBUG
    // vcpu_mem_access_debug(1, false);
    // printf("In peer cache: %d\n", in_peer_cache(63));
    // print_result();
    // return -1;
