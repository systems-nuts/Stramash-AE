...
#define SWITCH_ARM (uint64_t)0x1200000800
...
static void print_info(RunningInfo *info)
{
    ...
    double l1i_hit_rate = (double)info->l1i_hits / (info->l1i_hits + info->l1i_misses) * 100.0;
                            ...
                            uint64_t vaddr, void *userdata)
{
    ...
    {   
        ...
        // printf("now icount:      %lx\n",shmcatalog->prev_icount);
        // printf("sync insn_count: %lx\n",shmcatalog->insn_count);

        // printf("other icount:     %lx\n", other_shmcatalog->prev_icount);
        // printf("other insn_count: %lx\n", other_shmcatalog->insn_count);

        // printf("delta: %lx\n", shmcatalog->icount_base);
        // printf("other delta: %lx\n", other_shmcatalog->icount_base);
...
{   
    ...
    g_mutex_lock(&l1_dcache_locks[cache_idx]);
    hit_in_l1 = access_cache(l1_dcaches[cache_idx], insn_addr, &l1_ret);
    g_mutex_unlock(&l1_dcache_locks[cache_idx]);
    
    if (hit_in_l1 || !use_l2)
    {
        // L1i hit
        shmcatalog->running_info->l1i_hits++;
        return;
    }
    g_mutex_lock(&l2_ucache_locks[cache_idx]);
    hit_in_l2 = access_cache(l2_ucaches[cache_idx], insn_addr, &l2_ret);
    g_mutex_unlock(&l2_ucache_locks[cache_idx]);

    if (hit_in_l2 || !use_l3)
    {
        // L2 hit
        shmcatalog->running_info->l1i_misses++;
        shmcatalog->running_info->l2_hits++;
        return;
    }  
    ...
    if (shared_l3 == true)
    {
        ...
        hit_in_l3 = access_cache(l3_ucache, insn_addr, &l3_ret);
        g_mutex_unlock(l3_ucache_locks);
    }
    else{
        hit_in_l3 = access_cache(l3_ucache, insn_addr, &l3_ret);
    ...
    if (!hit_in_l3)
    {
        ...
        // l1_ret.blocks->state = EXCLUSIVE;
        shmcatalog->running_info->l1i_misses++;
        shmcatalog->running_info->l2_misses++;
        shmcatalog->running_info->l3_misses++;
    }
    else
    {
        // L3 hit
        ...
        shmcatalog->running_info->l1i_misses++;
        shmcatalog->running_info->l2_misses++;
        shmcatalog->running_info->l3_hits++;
    }
...
static void vcpu_tb_trans(qemu_plugin_id_t id, struct qemu_plugin_tb *tb)
{
    ...
    for (i = 0; i < n_insns; i++)
    {
        ...
        qemu_plugin_register_vcpu_insn_exec_cb(insn, vcpu_insn_exec,
                                               QEMU_PLUGIN_CB_NO_REGS, &effective_addr);
    }
}
...
{    
    ...
    for (i = 0; i < argc; i++)
    {
        ...
        else if (g_strcmp0(tokens[0], "test") == 0)
        {
            ...
            {
                printf("OPENPITON_MEM\n");
            }else if(test==SHARED_MEM)
            {
                printf("SHARED_MEM\n");
            }
