...
//		printk("%lx before pte %lx pte* %lx pmd %lx pmd* %lx \n",addr , pte, *pte, pmd, *pmd);
	...
	//	printk("%lx for fun pte %lx pte* %lx vmf pte %lx vmf *pte %lx entry %lx\n",addr , pte, *pte,vmf->pte,*vmf->pte,entry);
	//	printk("%lx physical addr pte %lx page %lx entry %lx \n",addr, (pte_val(*pte) & PTE_PFN_MASK), page_to_phys(page),__pte_to_phys(entry));
		set_pte(pte,entry);
/*
		printk("-------------pte %lx phys %lx\n",entry,__pte_to_phys(entry));
		struct page *fuck = (struct page*)ioremap(__pte_to_phys(entry), sizeof(struct page));
		printk("entry page %lx *page %lx pfn %lx phys address %lx\n",page,*page,pte_pfn(entry),page_to_phys(page));
		printk("ioremap entry phys %lx page %lx page* %lx\n",__pte_to_phys(entry),fuck,*fuck);
		fuck->pages = 123321;
                printk("%s ->flags  %ld ->pages %ld size %d\n", __func__,fuck->flags,fuck->pages,sizeof(*fuck));
		iounmap(fuck);
*/
//		printk("%lx successed make pte %lx pte* %lx  pmd %lx pmd* %lx \n",addr , pte, *pte, pmd, *pmd);
		return 0;
#endif
	}
//	struct page *mypage;
 //       mypage = (struct page*)ioremap((pte_val(*pte) &  PTE_PFN_MASK),sizeof(struct page));
//	printk(" ioremap page %lx *page %lx \n ",mypage, *mypage);
	set_pte(vmf->pte,pfn_pte((pte_val(*pte) & PTE_PFN_MASK) >> PAGE_SHIFT,PAGE_SHARED));
	//printk("%lx for fun pte %lx pte* %lx vmf pte %lx vmf *pte %lx\n",addr , pte, *pte,vmf->pte,*vmf->pte);
	pte_make_valid(*vmf->pte);	
//	pte_wrprotect(*vmf->pte);
	//printk("%lx for fun pte %lx pte* %lx\n",addr , pte, *pte);
...
#if 1 
	if(unlikely(test_page_owner(current->origin_nid,vmf->vma->vm_mm,addr)))
        {
                checker=1;
                pte_t entry;
...
#endif
	//for icount counter 
        trigger_address  = ioremap(ARCH_TRIGGER_ADDR, 0x1000);
        icount_switch();
		...
		set_page_owner(current->origin_nid, vmf->vma->vm_mm, addr);
                spin_unlock(ptl);
                pte_unmap_unlock(vmf->pte, ptl);
                fh->ret=0;
                __finish_fault_handling(fh);
