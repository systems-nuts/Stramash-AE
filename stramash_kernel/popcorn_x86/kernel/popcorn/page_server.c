...
unsigned long origin_pgd,remote_pgd;
...
inline bool PageStramash(struct mm_struct *mm, unsigned long addr)
{
    unsigned long offset;
    unsigned long *pi = __get_page_info_mapped(mm, addr, &offset);
    bool ret;
...
static int soft_page_walk(struct vm_fault *vmf)
{
	unsigned long addr = vmf->address & PAGE_MASK;
        ...
        pmd_t *pmd,*pmd_v;
        pte_t *pte,*pte_v;
        pgd = ioremap(remote_pgd,PAGE_SIZE);
        pgd = pgd_offset_popcorn(pgd,addr);
        //p4d = pgd;
 	pud_v = (pud_t *)__va((pte_val(__pte(pgd_val(*pgd))) & PTE_ADDR_LOW) + sizeof(pud_t) * ((addr >> PUD_SHIFT_POPCORN) & (PTRS_PER_PUD_POPCORN - 1)));
	pud=ioremap(__pa(pud_v),PAGE_SIZE);
	pmd_v = (pmd_t *)__va((pte_val(__pte(pud_val(*pud))) & PTE_ADDR_LOW) + sizeof(pmd_t) * ((addr >> PMD_SHIFT_POPCORN) & (PTRS_PER_PMD_POPCORN - 1)));
        pmd=ioremap(__pa(pmd_v),PAGE_SIZE);
	pte_v = (pte_t *)__va((pte_val(__pte(pmd_val(*pmd))) & PTE_ADDR_LOW) + sizeof(pte_t) * ((addr >> PAGE_SHIFT) & (PTRS_PER_PTE_POPCORN - 1)));
        pte=ioremap(__pa(pte_v),PAGE_SIZE);

        u64 paddr = PFN_PHYS((pte_val(*pte) & PTE_ADDR_LOW) >> PAGE_SHIFT);
	if(((pte_val(*pte) & PTE_ADDR_LOW) >> PAGE_SHIFT)==0)
        {
                iounmap(pte);
                iounmap(pmd);
                iounmap(pud);
        ...
        set_pte(vmf->pte,pfn_pte((pte_val(*pte) & PTE_ADDR_LOW) >> PAGE_SHIFT,PAGE_SHARED));
...
static pte_t *__get_pte_at_alloc(struct mm_struct *mm, struct vm_area_struct *vma, unsigned long addr, pmd_t **ppmd, spinlock_t **ptlp)
{
	...
	//printk("I walk myself addr %lx \n pgd %lx %lx \n p4d %lx %lx\n pud %lx %lx\n pmd %lx %lx\n pte %lx %lx\n",addr, pgd, *pgd, p4d, *(p4d), pud, *(pud), pmd, *(pmd),pte_offset_map(pmd, addr), pte_pfn(*pte));
	*ppmd = pmd;
	*ptlp = pte_lockptr(mm, pmd);
	return pte;
}
...
//	if(!current->did_mig) return 0;
//	if(test_page_owner(current->remote_nid,vma->vm_mm,addr))
//    {
//		printk("zap a remote page\n");		
//    }
...
again:
	...
	page = get_normal_page(vma, addr, pte); //we get the page, but we don't know if it is distributed or not
	BUG_ON(!page);
	// leader => true; the page fault had before, it is distributed, so we have to check the page owner to check should we update it even it is local page 
	if (leader) {
		pte_t entry;
		...
		//Page is owned by remote, which means the page has been distributed already, we have to update it even the page is local
		//Soft page walk shouldn't worry about this because we can access this page directly. 
		//However if origin need this page, it need claim it back. (Need to handle)

		if (test_page_owner(from_nid, mm, addr)) {
			BUG_ON(fault_for_read(fault_flags) && "Read fault from owner??"); 
			__claim_local_page(tsk, addr, from_nid); //Update the page that belong to me but is distributed? 
			grant = true; // Grant is true, means the remote has the latest page
					// Origin also updated (SOFT PAGE WALK need this, the origin solution doesn't do anything in this case)
		} else { //The page is not own by remote
			if (!page_is_mine(mm, addr)) { 
				//Update the page that belong to remote and it is distributed
				__claim_remote_page(tsk, mm, vma, addr, fault_flags, page);
			} else {
				//if there is a writing before, we need update the page, and it must be distributed otherwise no need.
				if (fault_for_write(fault_flags))
					__claim_local_page(tsk, addr, my_nid);
			}
		}
		
		// The page is first time be distributed...
		spin_lock(ptl);

		//Can we do this in remote? SOFT PAGE WALK -^- 

		SetPageDistributed(mm, addr);
		set_page_owner(from_nid, mm, addr);
		
		entry = ptep_clear_flush(vma, addr, pte);
		// If the page fault because of wrting
		if (fault_for_write(fault_flags)) { 
			// Remote is writing, I will invalide my page, I will not access it until next time I claim it.
			// In this case no need rewrite, because remote is write to this page in physical, it actually updated by remote.
			clear_page_owner(my_nid, mm, addr);
			...
			//Remote just reading, I can still use this page, but I better not write to this page because remote is reading. 
...
//	printk("I walk %lx pte %lx *pte %lx\n",addr,pte,*pte);
	pte_unmap(pte);
	//If grant => false, the page is not latest in remote, latest page is in host, either claim or not.
	if (!grant) {
		flush_cache_page(vma, addr, page_to_pfn(page)); 
		if (TRANSFER_PAGE_WITH_RDMA) {
			paddr = kmap(page);
