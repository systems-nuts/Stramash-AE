...
#define ALLOW_REMOTE_ALLOCATE_PAGE
// #define MULTI_PRIME
#ifdef MULTI_PRIME
#include <linux/io.h>
...
int migrated_checker;
EXPORT_SYMBOL(migrated_checker);

extern unsigned long origin_pgd;
...
{	
	...
	//			__func__, current->remote_pid, req->remote_pid);
		msleep(10);
		if (current->remote_pid != req->remote_pid)
		current->remote_pid= req->remote_pid;
	//	goto out_free;
	}
#ifdef MULTI_PRIME
//	icount_switch();
...
//	printk("start \n");
	...
	req->personality = tsk->personality;
	//TONG 
	//printk("mm->pgd pa %lx va %lx\n",virt_to_phys(mm->pgd),mm->pgd);
	req->origin_pgd = mm->pgd;
	req->origin_mm_structure = (unsigned long)&tsk->vmacache;
	//printk("vmacache va %llx\n",&tsk->vmacache);
	struct vm_area_struct *_vma = tsk->vmacache.vmas[0];
	req->origin_mm = (unsigned long)mm;
	//printk("origin mm %lx %ld \n",mm,mm->task_size);
//	printk("first vma cache pa %lx va %lx start %lx end %lx\n",virt_to_phys(_vma),_vma,_vma->vm_start,_vma->vm_end);

	req->origin_rb_node = tsk->mm->mm_rb.rb_node;
	struct rb_node *tmt = (struct rb_node*)ioremap(__pa(req->origin_rb_node),sizeof(struct rb_node)); 
	...
	*/
	...
	req->origin_lock = (unsigned long*)origin_lock;
...
//STRAMASH TAG PROCESS IS MIGRATED ALREADY
#ifdef ALLOW_REMOTE_ALLOCATE_PAGE
		//if(!tsk->did_mig)
		//	icount_switch();
		tsk->did_mig = 1; 
#endif
...
#endif
	origin_lock = kmalloc(sizeof(struct spinlock), GFP_KERNEL);
    spin_lock_init(origin_lock);
