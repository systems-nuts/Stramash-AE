...
#else
extern void *remote_address_space_high;
void *remote_address_space;
...
static int __init init_kmsg_shm(void)
{
	...
	*/
	...
	online_pages(0x100000,0x50000,MMOP_ONLINE_MOVABLE);
#endif
	printk("remote address space map\n");
#ifdef CONFIG_ARM64
	remote_address_space = ioremap(0x40000000, 0xc0000000);
    remote_address_space_high = ioremap(0x200000000, 0x1fffffff);
    printk("done, remote_address_space %lx\n",remote_address_space);
#else
	//IMPORTANT: if ARM allocate the memory in low. 
	//remote_address_space = ioremap(0x00000000, 0xb0000000);
    //remote_address_space_high = ioremap(0x210000000, 0xfffffff);
