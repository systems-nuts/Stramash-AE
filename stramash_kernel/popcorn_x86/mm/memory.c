			  ...
			  pte_t pte, struct page *page)
{
	...
	dump_stack();
	pr_alert("addr %lx BUG: Bad page map in process %s  pte:%08llx pmd:%08llx\n", addr,
		 current->comm,
...
/*
		...
		printk("old pte %lx %lx phys %lx\n",pte,pte_val(pte),(pte_val(pte) &  PTE_ADDR_LOW));
		
		pfn = pte_pfn(pfn_pte(((pte_val(pte) &  PTE_ADDR_LOW) >> PAGE_SHIFT),PAGE_SHARED));
		printk("new pte %lx %lx\n",pte_val(pfn_pte(((pte_val(pte) & PTE_ADDR_LOW) >> PAGE_SHIFT),PAGE_SHARED)),pfn_pte(((pte_val(pte) & PTE_ADDR_LOW) >> PAGE_SHIFT),PAGE_SHARED));
		struct page *mypage = (struct page*)ioremap((pte_val(pte) &  PTE_ADDR_LOW), 56);
		struct page *mypage2 = pfn_to_page(pfn);
		struct page *mypage3 = kmap(pfn_to_page(pfn));
		printk("1 *pages %lx ->pages %ld \n 2 *pages %lx ->pages %ld \n 3 *pages %lx ->pages %ld\n",
					*mypage, mypage->pages,*mypage2, mypage2->pages,*mypage3, mypage3->pages);
...
again:
   ...
   // {
//		page2 = pmd_page(*pmd);
 //       printk("%lx %lx ptl %lx mapcount %d\n ",pmd_pfn(*pmd), page_to_phys(page2), &page2->ptl, page_mapcount(page2));
//	}
	...
	do {
		...
		if (pte_present(ptent)) {
            ...
            {
                //Static solution
				//if the pte entry is in remote 
                if(pte_pfn(*pte)<=0x22ffff&&pte_pfn(*pte)>0x14ffff
                        || pte_val(*pte) < 0xffffe90000000000 && pte_val(*pte)>=0xffffe80000000000)
                {
	               // rss[mm_counter(page)]++;
					...
					continue; //without remove page stuff
                }
            }
			...
			if (unlikely(page_mapcount(page) < 0 && !current->did_mig))
		...
		if (unlikely(!free_swap_and_cache(entry) && !current->did_mig))
