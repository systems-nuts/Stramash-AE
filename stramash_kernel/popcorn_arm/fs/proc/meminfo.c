...
#include "internal.h"
#undef CONFIG_POPCORN_REMOTE_INFO
...
static int meminfo_proc_show(struct seq_file *m, void *v)
{
	...
	sunreclaim = global_node_page_state(NR_SLAB_UNRECLAIMABLE);
        ...
        rem_mem = send_remote_mem_info_request(0);
        if(rem_mem){

	show_val_kb(m, "MemTotal:       ", i.totalram);
	show_val_kb(m, "MemFree:        ", i.freeram);
	show_val_kb(m, "MemAvailable:   ", available);
	show_val_kb(m, "MemTotal remote:       ", rem_mem->MemTotal);
        show_val_kb(m, "MemFree remote:        ", rem_mem->MemFree);
        show_val_kb(m, "MemAvailable remote:   ", rem_mem->MemAvailable);
