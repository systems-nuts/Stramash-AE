...
again:
	...
	do {
		...
		if (pte_none(ptent))
			continue;
#ifdef CONFIG_POPCORN
		ret = page_server_zap_pte(vma, addr, pte, &ptent);
		if(ret==1)
		{
		 //Page is remote, I don't have to zap it
			continue;
		}
		if(ret==2)
		{
			//PAGE is stramash page
			//which means, I have my own pte, but I also write this pte to x86
			//because I'm zap, so x86 will not touch this memory anyway, 
			//so what I do it, I found the page, I can find it because this pte is mine
			//I flush the page, fluse the tlb, flush my pte. 
			//later on, x86 can just go and flush the pte. 
	        if (pte_present(ptent)) {
				...
				rss[mm_counter(page)]--;
				//only difference is this, because we can't touch the real pte.
