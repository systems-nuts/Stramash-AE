...
struct spinlock *origin_lock;

static int popcorn__signal;

EXPORT_SYMBOL(popcorn__signal);
extern unsigned long pre_pud_index;

extern pgd_t *origin_pgd;
extern struct vmacache *stramash_vmac;
extern struct rb_node *origin_rb_node;
extern unsigned long origin_rb_node_virt_address;
//#define Stramash_share_futex
#ifdef Stramash_share_futex
extern struct futex_hash_bucket *stramash_futex_queues;
extern unsigned long stramash_hashsize;
extern void stramash_futex_share(void);
extern void stramash_futex_share_off(void);
#endif
...
//////////////////////
#include <linux/io.h>
#define ARCH_TRIGGER_ADDR_OPEN    0x0f300000
#define ARCH_TRIGGER_ADDR_EXIT	  0x0f400000
#define ARCH_TRIGGER_ADDR_STOP    0x0f500000
static void __iomem *trigger_address_open;
static void __iomem *trigger_address_stop;
static void __iomem *trigger_address_exit;

static void inline cache_sim_open(void)
{
        readl(trigger_address_open);
}
static void inline cache_sim_stop(void)
{
        readl(trigger_address_stop);
}
static void inline cache_sim_exit(void)
{
        readl(trigger_address_exit);
}
//////////////////////

struct mm_struct *stramash_origin_mm;
EXPORT_SYMBOL(stramash_origin_mm);
...
//If shared virtual address, no need ioremap again.
#if 0
	iounmap(origin_rb_node);
#endif
	return 0;
}
...
struct vm_area_struct **_origin_mm_structure;
/* shared virtual address
void popcorn_update_vma(void)
{
	int i;
        for(i=0; i<VMACACHE_SIZE;i++)
        {
                origin_mm_structure[i] =  (struct vm_area_struct*)ioremap(((unsigned long)(*(_origin_mm_structure + i)) - 0xffff888000000000UL),sizeof(struct vm_area_struct));
	//with shared virtual address
        }
}
*/
//EXPORT_SYMBOL(popcorn_update_vma);
static int tmp_pid;
static struct task_struct *tmp_tsk;
static int remote_thread_main(void *_args)
{
	...
	*/
	/*Stramash share futex*/
#if 0
	stramash_futex_queues=(struct futex_hash_bucket*)req->origin_futex_queues;
	stramash_hashsize=req->origin_futex_hashsize;
	//printk("Remote futex_queues: %lx futex_hashsize %ld\n",stramash_futex_queues,stramash_hashsize);
	//stramash_futex_share();
#endif
	/*share futex done*/
	__pair_remote_task();
	/*Stramash */
	//origin_pgd = ioremap(req->origin_pgd,PAGE_SIZE);
	origin_pgd = req->origin_pgd; 
	origin_lock = (struct spinlock*)req->origin_lock;	

//	_origin_mm_structure = (struct vm_area_struct**)ioremap(req->origin_mm_structure,sizeof(struct vm_area_struct*)*4);

	stramash_vmac = (struct vmacache *)req->origin_mm_structure;
//	struct vm_area_struct *_vma = stramash_vmac->vmas[0];
//       printk("stramash_vmac %lx va %lx start %lx end %lx\n",stramash_vmac,_vma,_vma->vm_start,_vma->vm_end); 


	


	stramash_origin_mm= (struct mm_struct*)req->origin_mm;
//	printk("check if I can access, %ld %ld\n",stramash_origin_mm->task_size,req->task_size);
//	printk("let's try mm->pgd then pa %lx va %lx\n",virt_to_phys(stramash_origin_mm->pgd),stramash_origin_mm->pgd);
//	popcorn_update_vma();
	pre_pud_index = -1;
	origin_rb_node_virt_address = req->origin_rb_node;
	//If shared virtual address, no need ioremap again. 
	
#if 0
	origin_rb_node = (struct rb_node*)ioremap(((unsigned long)(req->origin_rb_node) - 0xffff888000000000UL),sizeof(struct rb_node));;
#else
	origin_rb_node = (struct rb_node*)(req->origin_rb_node);
#endif
//	printk("origin_rb_node color %lx\n",origin_rb_node->__rb_parent_color);
	/*end of Stramash*/
	PSPRINTK("\n####### MIGRATED - [%d/%d] from [%d/%d]\n",
			current->pid, my_nid, current->origin_pid, current->origin_nid);
	//icount_switch();
	//STRAMASH FOR IDENTIFY THE MIGRATION PROCESSS
	current->did_mig = 1; 
	//printk("start\n");	
	kfree(params);
	pcn_kmsg_done(req);
...
static int __fork_remote_thread(clone_request_t *req)
{
	...
	while (kernel_thread(remote_thread_main, params,CLONE_THREAD | CLONE_SIGHAND | SIGCHLD) < 0) {
		schedule();
	}
	return 0;
}
/* TODO:
 * SHM version, the remote->mm = req->mm 
*/
...
//the main handler thread in host, it will end up in post.
...
int request_remote_work(pid_t pid, struct pcn_kmsg_message *req)
{
	...
	if (!tsk) {
		printk(KERN_INFO"%s: invalid origin task %d for remote work %d PCN_KMSG_TYPE_VMA_OP_RESPONSE %d \n",
				__func__, pid, req->header.type, PCN_KMSG_TYPE_VMA_OP_RESPONSE);
		goto out_err;
...
//the main handler thread in remote, it will end up in post.
...
static int __request_clone_remote(int dst_nid, struct task_struct *tsk, void __user *uregs)
{
	...
	req->personality = tsk->personality;
	//TONG 
	req->origin_pgd = virt_to_phys(mm->pgd);
	req->origin_mm_structure = virt_to_phys(mm);
...
{
	trigger_address_open = ioremap(ARCH_TRIGGER_ADDR_OPEN, 0x1000);
	trigger_address_stop = ioremap(ARCH_TRIGGER_ADDR_STOP, 0x1000);
	trigger_address_exit = ioremap(ARCH_TRIGGER_ADDR_EXIT, 0x1000);
