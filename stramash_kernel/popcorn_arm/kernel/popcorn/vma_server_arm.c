...
#define CONFIG_REMOTE_VMA_FIND
#define SHARED_VIRTUAL_ADDRESS
extern void popcorn_update_vma(void);
struct vm_area_struct *origin_mm_structure[VMACACHE_SIZE];
EXPORT_SYMBOL(origin_mm_structure);
struct rb_node *origin_rb_node;
EXPORT_SYMBOL(origin_rb_node);
unsigned long origin_rb_node_virt_address;
EXPORT_SYMBOL(origin_rb_node_virt_address);
enum vma_op_code {
	VMA_OP_NOP = -1,
	VMA_OP_MMAP,
	VMA_OP_MUNMAP,
...
extern unsigned long remote_pgd;
...
static struct vma_info *__alloc_vma_info_request(struct task_struct *tsk, unsigned long addr, vma_info_request_t **preq)
{
	...
	req->remote_pgd=virt_to_phys(tsk->mm->pgd);
	/* req */
...
struct path *remote_file_path;
static int popcorn_vma_find(struct vma_info *vi, unsigned long address)
{
#ifndef CONFIG_REMOTE_VMA_FIND
	return 1;
#endif
	int idx = VMACACHE_HASH(address);
        int i;
	vma_info_response_t *res = NULL;
	while (!res) {
                res = kmalloc(sizeof(*res), GFP_KERNEL);
		...
		//printk("addr %lx vm_start %lx vm_end %lx\n",address,origin_mm_structure[idx]->vm_start,origin_mm_structure[idx]->vm_end);
 	        if(address>=origin_mm_structure[idx]->vm_start &&  address<=origin_mm_structure[idx]->vm_end)
                {
	//		printk("vma found in cache\n");
        		res->addr = address;
			res->result=0;
			res->vm_start=origin_mm_structure[idx]->vm_start;
			res->vm_end=origin_mm_structure[idx]->vm_end;
			res->vm_flags=origin_mm_structure[idx]->vm_flags;
			res->vm_pgoff=origin_mm_structure[idx]->vm_pgoff;
			res->remote_pid = current->pid;
	//		printk("vm_start %lx vm_end %lx\n vm_flags %lx, vm_pgoff %lx\n",origin_mm_structure[idx]->vm_start,origin_mm_structure[idx]->vm_end,origin_mm_structure[idx]->vm_flags,origin_mm_structure[idx]->vm_pgoff);
			//get_file_path(origin_mm_structure[idx]->vm_file, res->vm_file_path, sizeof(vi->response->vm_file_path));
			strncpy(res->vm_file_path,"",sizeof(vi->response->vm_file_path));
	//		printk("vm_file_pat %s\n",res->vm_file_path);
			vi->response=res;
			return 0;
                }
	...
	///printk("vma not found in cache!\n");

	struct rb_node *rb_node = origin_rb_node;
	// virt address of origin_rb_node, not in my address space 
	struct rb_node *rb_node_origin = origin_rb_node_virt_address;
	// the target VMA; 
	unsigned long tmp_address;
	struct vm_area_struct *rb_vma;
	while(rb_node)
	{
#ifdef	SHARED_VIRTUAL_ADDRESS
		// tmp VMA
                struct vm_area_struct *tmp;
                // get tmp VMA by use offset to calculate virt address of VMA contain the origin_rb_node
                // However, the VMA address is in remote address space
		// with shared Virtual address enable, it is in our address space
                tmp_address =(unsigned long)rb_node_origin - offsetof(struct vm_area_struct,vm_rb);

                // we can directly use tmp.
		tmp=(struct vm_area_struct*)tmp_address;
                // if the result in left
                if (tmp->vm_end > address) {
                        // assign the tmp to target vma, since rb tree alwasy has left most node
                        rb_vma=tmp;
                        // if the address fall in correct range, break and return vma.
                        if (tmp->vm_start <= address)
                                break;
                        // else the rb_node is rb_left, the virtual address(remote) of rb_left assigned
                        rb_node_origin = rb_node->rb_left;
                } else
                        //if the resilt in right, the virtual address(remote) of rb_right assigned
                        rb_node_origin = rb_node->rb_right;
		rb_node= rb_node_origin;
#else
		// tmp VMA
		struct vm_area_struct *tmp;
		// get tmp VMA by use offset to calculate virt address of VMA contain the origin_rb_node
		...
		// we can't directly use tmp.
	//	printk("remote virtual address vma %lx rb_node %lx\n",tmp,rb_node_origin);
		// ioremap this tmp by convert it to physical address first
		tmp = (struct vm_area_struct*)ioremap((tmp_address - 0xffff888000000000UL),sizeof(struct vm_area_struct));
		//because va is not overlapped, so we first convert tmp_address to phys by minus offset of x86, then just do __va of arm
		printk("tmp_address %lx, tmp_paddr: %lx, after __va %lx \n",tmp_address,(tmp_address - 0xffff888000000000UL),__va((tmp_address - 0xffff888000000000UL)));
		//tmp =  (struct vm_area_struct*)__va((tmp_address - 0xffff888000000000UL));
	//	printk("tmp vma, start %lx end %lx \n",tmp->vm_start,tmp->vm_end);
		// if the result in left
		if (tmp->vm_end > address) {
			// assign the tmp to target vma, since rb tree alwasy has left most node
			rb_vma=tmp;
		...
		rb_node = (struct rb_node*)ioremap(((unsigned long)(rb_node_origin) - 0xffff888000000000UL),sizeof(struct rb_node));
#endif
	}
	if(rb_vma)
	{
	//	printk("rb_tree search found VMA, start %lx end %lx\nvm_flags %lx, vm_pgoff %lx\n",rb_vma->vm_start,rb_vma->vm_end,rb_vma->vm_flags,rb_vma->vm_pgoff);
		res->addr = address;
                res->result=0;
                res->vm_start=rb_vma->vm_start;
                res->vm_end=rb_vma->vm_end;
                res->vm_flags=rb_vma->vm_flags;
                res->vm_pgoff=rb_vma->vm_pgoff;
                res->remote_pid = current->pid;
		strncpy(res->vm_file_path,"/root/tong",sizeof(vi->response->vm_file_path));
		//struct vm_area_struct *tmp = rb_entry(current->mm->mm_rb.rb_node, struct vm_area_struct, vm_rb);
		//get_file_path(tmp->vm_file, res->vm_file_path, sizeof(vi->response->vm_file_path));
		//printk("vm_file_path %s\n",res->vm_file_path);
                vi->response=res;
		
                return 0;
	}
	printk("BUG: Remote red-black tree search for VMA failed!\n");
	kfree(res);
	return 1;
}
/**
...
int vma_server_fetch_vma(struct task_struct *tsk, unsigned long address)
{
	...
	if (req) {
		...
		/*experiment*/
		//printk("vm_area_struct %ld\n",sizeof(struct vm_area_struct));
		/*experiment done*/
		if(popcorn_vma_find(vi,addr))
                {
                
		pcn_kmsg_send(PCN_KMSG_TYPE_VMA_INFO_REQUEST,
				tsk->origin_nid, req, sizeof(*req));
		wait_for_completion(&vi->complete);
		//printk("MSG: vm_start %lx vm_end %lx\n vm_flags %lx, vm_pgoff %lx\n",vi->response->vm_start,vi->response->vm_end,vi->response->vm_flags,vi->response->vm_pgoff);
		//printk("vm_file_pat %s\n",vi->response->vm_file_path);
			 popcorn_update_vma();
		}
		ret = vi->ret =
