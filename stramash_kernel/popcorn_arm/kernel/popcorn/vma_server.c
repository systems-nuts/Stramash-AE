...
struct vmacache *stramash_vmac;
EXPORT_SYMBOL(stramash_vmac);
...
static int popcorn_vma_find(struct vma_info *vi, unsigned long address)
{
	...
	while (!res) {
		...
		rb_vma = stramash_vmac->vmas[idx];
		if(rb_vma)
		{
 	        if(address>= rb_vma->vm_start &&  address<= rb_vma->vm_end)
                {
			...
			get_file_path(rb_vma->vm_file, res->vm_file_path, sizeof(vi->response->vm_file_path));
