...
const int MAX_SHM_SLICES=1;
#ifdef CONFIG_ARM64
static int popcorn_shm_slices = 0;
#else
static int popcorn_shm_slices = MAX_SHM_SLICES;
#endif
const unsigned long popcorn_shm_hotplug_size=0x100000;
const unsigned long popcorn_shm_start_addr=0x100000;
const unsigned long popcorn_shm_end_addr=0x200000;
EXPORT_SYMBOL(MAX_SHM_SLICES);
EXPORT_SYMBOL(popcorn_shm_slices);
EXPORT_SYMBOL(popcorn_shm_hotplug_size);
EXPORT_SYMBOL(popcorn_shm_start_addr);
EXPORT_SYMBOL(popcorn_shm_end_addr);


int get_popcorn_shm_slices(void)
{
	return popcorn_shm_slices;
}
EXPORT_SYMBOL(get_popcorn_shm_slices);
...
EXPORT_SYMBOL(send_remote_mem_info_request);
...
/*Physical memory allocator handler*/
static int fill_mem_reassign_response(mem_reassign_response_t *res)
{
	if(popcorn_shm_slices==0)
	{
		res->success=false;
		return 0;
	}
	res->offline_start=popcorn_shm_start_addr + (popcorn_shm_slices-1)*popcorn_shm_hotplug_size;
	if(!offline_pages(res->offline_start,popcorn_shm_hotplug_size))
	{
		popcorn_shm_slices-=1;
		res->offline_size=popcorn_shm_hotplug_size;
		res->success=true;
	}
	else
	{
		printk("offline failed\n");
                res->success=false;
                return 0;
        }
}
...
{
	START_KMSG_WORK(mem_reassign_request_t, request, work);
        mem_reassign_response_t response = {
                .nid = my_nid,
                .origin_ws = request->origin_ws,
        };
        int ret;

        ret = fill_mem_reassign_response(&response);
        if (ret < 0) {
                RIPRINTK("%s: failed to reassign memory info\n", __func__);
                goto out;
        }
        ret = pcn_kmsg_send(PCN_KMSG_TYPE_MEM_REASSIGN_RESPONSE,
                        request->nid, &response, sizeof(response));
        if (ret < 0) {
                RIPRINTK("%s: failed to send response message\n", __func__);        
                goto out;
...
{
	mem_reassign_response_t *response = (mem_reassign_response_t *)inc_msg;
        struct wait_station *ws;
...
{
	mem_reassign_request_t request = {
                .nid = my_nid,
        };
        mem_reassign_response_t *response;
        struct wait_station *ws = get_wait_station(current);
        ...
        request.origin_ws = ws->id;
        pcn_kmsg_send(PCN_KMSG_TYPE_MEM_REASSIGN_REQUEST,
                        nid, &request, sizeof(request));
        response = wait_at_station(ws);

        return response;
}
int __do_hot_plug(mem_reassign_response_t *mem_reassign)
{       
        int ret;
        if(!mem_reassign->success) return 1;
        //printk("oneline %lx %lx\n",mem_reassign->offline_start,mem_reassign->offline_size);
        ret=online_pages(mem_reassign->offline_start,mem_reassign->offline_size,MMOP_ONLINE_MOVABLE);       
        if(!ret)
        {       
                //printk("oneline successful\n");
                popcorn_shm_slices+=1;
                return 0;
        }
        else return 1;
}
void do_hot_plug(mem_reassign_response_t *mem_reassign)
{
	__do_hot_plug(mem_reassign);
}
EXPORT_SYMBOL(do_hot_plug);


EXPORT_SYMBOL(send_mem_reassign_request);

DEFINE_KMSG_WQ_HANDLER(remote_cpu_info_request);
DEFINE_KMSG_WQ_HANDLER(remote_mem_info_request);
DEFINE_KMSG_WQ_HANDLER(mem_reassign_request);
int remote_info_init(void)
{
	...
	REGISTER_KMSG_WQ_HANDLER(PCN_KMSG_TYPE_MEM_REASSIGN_REQUEST,
                                   mem_reassign_request);
	REGISTER_KMSG_HANDLER(PCN_KMSG_TYPE_MEM_REASSIGN_RESPONSE,
                                   mem_reassign_response);
	return 0;
}
