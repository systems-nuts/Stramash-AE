...
//DEBUG page fault checker
static int allocated_pages = 0; 
extern int remote_page_fault;
...
//Remote page walker 
#define CONFIG_SOFT_PAGE_WALK

#define CONFIG_SHARED_PTL
extern struct mm_struct *stramash_origin_mm;
#define CONFIG_SHARED_VIRTUAL_ADDRESS

void *remote_address_space, *remote_address_space_high;
unsigned long remote_pgd;
pgd_t *origin_pgd;
unsigned long pre_pud_index;
int ioremap_total=0, ioremap_bypass=0, ioremap_fucked=0;
EXPORT_SYMBOL(remote_address_space);
EXPORT_SYMBOL(remote_address_space_high);
EXPORT_SYMBOL(ioremap_bypass);
EXPORT_SYMBOL(ioremap_total);
EXPORT_SYMBOL(ioremap_fucked);
EXPORT_SYMBOL(pre_pud_index);
EXPORT_SYMBOL(origin_pgd);
EXPORT_SYMBOL(remote_pgd);
...
//FOR STRAMASH, temporary, need refactor the Redix tree, but don't want work too much
//regular use still work, so the we still put correct nid to bit 1 or 0. 
//however we should able to distinguish. 
static inline void SetPageStramash(struct mm_struct *mm, unsigned long addr)
{
    unsigned long key, offset;
    unsigned long *region;
    struct page *page;
    ...
    page = __lookup_page_info_page(rc, key);
    region = kmap_atomic(page);
    set_bit(PI_FLAG_STRAMASH, region + offset);
    kunmap_atomic(region);
}

static inline bool PageStramash(struct mm_struct *mm, unsigned long addr)
{
    unsigned long offset;
    unsigned long *pi = __get_page_info_mapped(mm, addr, &offset);
    bool ret;
    ...
    if (!pi) return false;
    ret = test_bit(PI_FLAG_STRAMASH, pi);
    kunmap_atomic(pi - offset);
    return ret;
}
...
// SOFTWARE PAGEWALKER SUPPORT
#define PAGE_OFFSET_x86		_AC(0xffff888000000000, UL)
#define PGDIR_SHIFT_POPCORN     39
#define PTRS_PER_PGD_POPCORN    512
#define pgd_index_popcorn(addr)         (((addr) >> PGDIR_SHIFT_POPCORN) & (PTRS_PER_PGD_POPCORN - 1))
#define pgd_offset_raw_popcorn(pgd, addr)       ((pgd) + pgd_index_popcorn(addr))
#define pgd_offset_popcorn(pgd, addr)   (pgd_offset_raw_popcorn(pgd, (addr)))
#define __PHYSICAL_MASK_SHIFT	52
#define PMD_SHIFT_POPCORN	21
#define PUD_SHIFT_POPCORN	30
#define PTRS_PER_PUD_POPCORN	512
#define PTRS_PER_PMD_POPCORN	512
#define PTRS_PER_PTE_POPCORN	512
#define PAGE_MASK_POPCORN	(~(PAGE_SIZE-1))
#define PGDIR_SIZE_POPCORN	(_AC(1, UL) << PGDIR_SHIFT_POPCORN)
#define PGDIR_MASK_POPCORN	(~(PGDIR_SIZE_POPCORN - 1))

#define __PHYSICAL_MASK 	(1ULL << __PHYSICAL_MASK_SHIFT) - 1
#define PMD_PAGE_SIZE		(_AC(1, UL) << PMD_SHIFT_POPCORN)
#define PMD_PAGE_MASK		(~(PMD_PAGE_SIZE-1))
#define PUD_PAGE_SIZE		(_AC(1, UL) << PUD_SHIFT_POPCORN)
#define PUD_PAGE_MASK		(~(PUD_PAGE_SIZE-1))

#define PHYSICAL_PAGE_MASK	(((signed long)PAGE_MASK_POPCORN) & __PHYSICAL_MASK)

#define PHYSICAL_PMD_PAGE_MASK	(((signed long)PMD_PAGE_MASK) & __PHYSICAL_MASK)
#define PHYSICAL_PUD_PAGE_MASK	(((signed long)PUD_PAGE_MASK) & __PHYSICAL_MASK)
#define PTE_PFN_MASK		((pteval_t)PHYSICAL_PAGE_MASK)
#define __va_popcorn(x)		((unsigned long)((x) + PAGE_OFFSET_x86))
#define __pa_popcorn(x)		(phys_addr_t)x - PAGE_OFFSET_x86 
/*
static u64 search_for_page(unsigned long addr)
{
	
}
*/

static pud_t *pre_pud=NULL,*pud,*pud_v,*pud_test;
static pmd_t *pre_pmd=NULL,*pmd,*pmd_v;
static pte_t *pte,*pte_v;
static p4d_t *p4d;
static unsigned long start_point;
static unsigned long pre_pmd_index = -1;
static void* remote_address;
#ifdef CONFIG_SHARED_VIRTUAL_ADDRESS 
#include <asm/pgalloc.h>
//RETURN 0 FOR ERROR
//RETURN 1 FOR CORRECT READ
//RETURN 2 FOR ALLOCATE PAGE IN REMOTE
static int soft_page_walk(struct vm_fault *vmf)
{
#ifdef CONFIG_SHARED_PTL
	struct spinlock *ptl;
#endif
	unsigned long addr = vmf->address & PAGE_MASK;
        p4d = pgd_offset_popcorn(origin_pgd,addr);
	if(p4d_val(*p4d)==0)
        {
                return 0;
        }

	if(unlikely(pre_pud_index!= ((addr >> PUD_SHIFT_POPCORN) & (PTRS_PER_PUD_POPCORN - 1))))
	{
		pre_pmd_index = -1;
		pud=pre_pud = (pud_t *)(unsigned long)__va_popcorn(p4d_val(*p4d) & PTE_PFN_MASK) + ((addr >> PUD_SHIFT_POPCORN) & (PTRS_PER_PUD_POPCORN - 1));
		pre_pud_index= ((addr >> PUD_SHIFT_POPCORN) & (PTRS_PER_PUD_POPCORN - 1));
	}
	else
	{
		pud=pre_pud;
	...
	if(unlikely(pre_pmd_index!= ((addr >> PMD_SHIFT_POPCORN) & (PTRS_PER_PMD_POPCORN - 1))))
	{
		pmd=pre_pmd = (pmd_t *)(unsigned long)__va_popcorn(pud_val(*pud) & PTE_PFN_MASK) + ((addr >> PMD_SHIFT_POPCORN) & (PTRS_PER_PMD_POPCORN - 1));
		pre_pmd_index=((addr >> PMD_SHIFT_POPCORN) & (PTRS_PER_PMD_POPCORN - 1));
	}
	else
	{
		pmd=pre_pmd;
	...
	pte= (pte_t *)(unsigned long)__va_popcorn(pmd_val(*pmd) & PTE_PFN_MASK) + ((addr >> PAGE_SHIFT) & (PTRS_PER_PTE_POPCORN - 1));

	//if(fault_for_write(vmf->flags))
	//	printk("write %lx pte %lx pmd %lx pud %lx pgd %lx \n",addr , *pte, *pmd, *pud, *p4d);
	//else
	//	printk("read %lx pte %lx pmd %lx pud %lx pgd %lx \n",addr , *pte, *pmd, *pud, *p4d);
	if(unlikely(((pte_val(*pte) & PTE_PFN_MASK) >> PAGE_SHIFT)==0))
	{
	/*Page is not allocated, try allocated for it */
	/* Updated: first, also in origin side, there is no page table entry,
	*  If we just assign our side, once the application migrate back, this would 
	*  cause a trouble, thus, we should allocated such pte in vmalloc range, which 
	*  address is alligning with the origin. 
	*/
#if 0
		return 0; 
#else
//		printk("%lx before pte %lx pte* %lx pmd %lx pmd* %lx \n",addr , pte, *pte, pmd, *pmd);

		pte_t entry;
			
		struct page *page = alloc_zeroed_user_highpage_movable(vmf->vma, addr);
		struct page *page2;
		if (!page)
                	return 0;
		allocated_pages+=1;
		//printk("allocated page at %lx\n",addr);
		__SetPageUptodate(page);
		entry = mk_pte(page, vmf->vma->vm_page_prot);
		/* 
		mk_pte actually make a pte in the remote side default address space
		in our case, 0xffffe000000000 however, we should either map it or copy it 
		to 0xffff88800000000, or, we create such pte in the origin address part directly.
		*/
		if (vmf->vma->vm_flags & VM_WRITE)
            entry = pte_mkwrite(pte_mkdirty(entry));
		if (pte_none(entry)) 
			return 0;
		set_pte_at(vmf->vma->vm_mm, addr, vmf->pte, entry);
		update_mmu_cache(vmf->vma, addr, vmf->pte);
		pte_make_valid(*vmf->pte);

		atomic_set(&(page)->_mapcount, 0);	
		//We should lock here because it was modify the pte of x86.

		ptl = &stramash_origin_mm->page_table_lock;

		spin_lock(ptl);
		set_pte(pte,entry);
		spin_unlock(ptl);
		
	//	printk("addr ptl %lx\n",&stramash_origin_mm->page_table_lock);
	...
	//	if(fault_for_write(vmf->flags))
      //      printk("%lx pte %lx pmd %lx pud %lx pgd %lx pft %lx\n",addr , *pte, *pmd, *pud, *p4d, pte_pfn(*pte));
		return 2;
#endif
	}
	set_pte(vmf->pte,pfn_pte((pte_val(*pte) & PTE_PFN_MASK) >> PAGE_SHIFT,PAGE_SHARED));
	pte_make_valid(*vmf->pte);	
	return 1;
}
#else
static int soft_page_walk(struct vm_fault *vmf)
{
        ...
        {
                pre_pmd_index = -1;
                pud_v = (pud_t *)(unsigned long)__va(p4d_val(*p4d) & PTE_PFN_MASK) + ((addr >> PUD_SHIFT_POPCORN) & (PTRS_PER_PUD_POPCORN - 1));
               // printk("pud_v %lx %lx %lx\n",pud_v,__pa(pud_v),__pa_popcorn(pud_v));
                if(unlikely(__pa(pud_v) >= 0x100000000))
                {
                        remote_address=remote_address_space_high;
                        start_point = 0x100000000;
                }
                else
                {
                        remote_address=remote_address_space;
                        start_point = 0x0;
                }
                pud=pre_pud=(pud_t *)(remote_address + (__pa(pud_v)-start_point));
                //printk("pud %lx %lx %lx\n",pud,__pa(pud),__pa_popcorn(pud));
                pre_pud_index= ((addr >> PUD_SHIFT_POPCORN) & (PTRS_PER_PUD_POPCORN - 1));
        }
        else
        {
        ...
        if(unlikely(pre_pmd_index!= ((addr >> PMD_SHIFT_POPCORN) & (PTRS_PER_PMD_POPCORN - 1))))
        {
                pmd_v = (pmd_t *)(unsigned long)__va(pud_val(*pud) & PTE_PFN_MASK) + ((addr >> PMD_SHIFT_POPCORN) & (PTRS_PER_PMD_POPCORN - 1));
        //      printk("pmd_v %lx %lx\n",pmd_v,__pa(pmd_v));
                if(unlikely(__pa(pmd_v) >= 0x100000000))
                {
                        remote_address=remote_address_space_high;
                        start_point = 0x100000000;
                }
                ...
                pmd=pre_pmd=(pmd_t *)(remote_address + (__pa(pmd_v)-start_point));
        //      printk("pmd_v %lx %lx\n",pmd,__pa(pmd));
                pre_pmd_index=((addr >> PMD_SHIFT_POPCORN) & (PTRS_PER_PMD_POPCORN - 1));
        }
        else
        ...
        pte_v = (pte_t *)(unsigned long)__va(pmd_val(*pmd) & PTE_PFN_MASK) + ((addr >> PAGE_SHIFT) & (PTRS_PER_PTE_POPCORN - 1));
        //printk("pte_v %lx %lx\n",pte_v,__pa(pte_v));
        if(unlikely(__pa(pte_v) >= 0x100000000))
        {
                remote_address=remote_address_space_high;
                start_point = 0x100000000;
        }
        ...
        pte=(pte_t *)(remote_address + (__pa(pte_v)-start_point));
        //printk("pte %lx %lx\n",pte,__pa(pte));
        if(unlikely(((pte_val(*pte) & PTE_PFN_MASK) >> PAGE_SHIFT)==0))
        {
        /*Page is not allocated, try allocated for it */
                ...
                __SetPageUptodate(page);
                entry = mk_pte(page, vmf->vma->vm_page_prot);
        //      printk("entry: pte %lx *pte %lx pte_val(*pte) %lx\n",&entry,entry,pte_val(entry));
                if (vmf->vma->vm_flags & VM_WRITE)
                        entry = pte_mkwrite(pte_mkdirty(entry));
                if (pte_none(entry))
                        return 1;
...
static pte_t *__get_pte_at(struct mm_struct *mm, unsigned long addr, pmd_t **ppmd, spinlock_t **ptlp)
{
	pgd_t *pgd;
	p4d_t *p4d;
	pud_t *pud,*pud_v5,*pud_v1,*pud_v2,*pud_v3,*pud_v4;
	pmd_t *pmd;
	...
	*ptlp = pte_lockptr(mm, pmd);
	//printk("I walk myself addr %lx \n pgd %lx %lx \n p4d %lx %lx\n pud %lx %lx\n pmd %lx %lx\n pte %lx %lx\n",addr, pgd, *pgd, p4d, *(p4d), pud, *(pud), pmd, *(pmd),pte_offset_map(pmd, addr), pte_pfn(*pte_offset_map(pmd, addr)));
...
static remote_page_response_t *__fetch_page_from_origin(struct task_struct *tsk, struct vm_area_struct *vma, unsigned long addr, unsigned long fault_flags, struct page *page)
{
	...
	//remote give me the page, otherwise it is VM_FAULT_CONTINUE...
	if (rp->result == 0) {
		...
		} else 
		{
			...
			//new_addr = ioremap(phys_addr,PAGE_SIZE);
			else
		        new_addr = ioremap(rp->phys_addr+0x40000000,PAGE_SIZE);	
			//new_addr = ioremap(phys_addr+0x40000000,PAGE_SIZE);
			u64* karim = new_addr;
	...
	//printk("rp->result: %ld",rp->result);
	return rp;
}

static remote_page_response_t *__fetch_page_from_origin_fake(struct task_struct *tsk, struct vm_area_struct *vma, unsigned long addr, unsigned long fault_flags, struct page *page)
{
        remote_page_response_t *rp;
        void * new_addr;
        struct wait_station *ws = get_wait_station(tsk);
...
//return 2 for handling the stramash page
int page_server_zap_pte(struct vm_area_struct *vma, unsigned long addr, pte_t *pte, pte_t *pteval)
{
	if (!vma->vm_mm->remote) return 0; 
	if(PageStramash(vma->vm_mm, addr))
	{
		return 2;
	}

	if(test_page_owner(current->origin_nid,vma->vm_mm,addr))
    {
        return 1;
    }
...
static int __handle_remotefault_at_remote(struct task_struct *tsk, struct mm_struct *mm, struct vm_area_struct *vma, remote_page_request_t *req, remote_page_response_t *res)
{
	...
	if (TRANSFER_PAGE_WITH_RDMA) {
		...
		//printk("I walk %lx, page %lx %lx for you -> %lx\n",addr, page, __pa(page) ,res->phys_addr);
 ...
 */
//Characterize the remote page walker
#include <linux/io.h>
...
static void __iomem *trigger_address;
static void inline icount_switch(void)
{
...
////////////////////////////////////
// SOFT PAGE WALK START
        int checker=0;
        //printk("__handle_localfault_at_remote I will try walk %lx myself\n",addr);
#ifdef CONFIG_SOFT_PAGE_WALK
#ifdef CONFIG_SHARED_VIRTUAL_ADDRESS 
//WITH SHARED VIRTUAL ADDRESS ENABLE
//SOFT_PAGE_WALK WILL RETURN 3 CASE 
//0 FOR ERROR, 1 FOR SUCCESS READ, 2 FOR SUCCESS AND ALLOCATE PAGE 
		ret = soft_page_walk(vmf);	
	    if(likely(ret))
        {
        //  icount_switch();
        //  iounmap(trigger_address);
			...
			if(ret==2)
				SetPageStramash(vmf->vma->vm_mm, addr);
			
			set_page_owner(current->remote_nid, vmf->vma->vm_mm, addr); 
            spin_unlock(ptl);
            pte_unmap_unlock(vmf->pte, ptl);
            ioremap_fucked++;
            fh->ret=0;
...
#endif
#endif
//	printk("Amazing! Something goes wrong MSG\n");
	if (!fh) {
		pte_unmap(vmf->pte);
	...
	page = vm_normal_page(vmf->vma, addr, *vmf->pte);
        ...
        get_page(page); //I get the Page, however this page is remote.
	//I need fetch the page from origin, I have sent the message
	rp = __fetch_page_from_origin(current, vmf->vma, addr, vmf->flags, page);
	//If the response is shit, page can't be found in origin, sucks, die die die!
	...
	// Origin told me continue, in this case, Origin has update the latest page from me, claim local or remote, I have the latest 
	// page in this case
	if (rp->result == VM_FAULT_CONTINUE) {
		...
		// However, if the fault is becasue of writing, because I have latest freash page, I just write to it. 
		if (fault_for_write(vmf->flags)) {
			entry = pte_mkwrite(entry);
			entry = pte_mkdirty(entry);
		} else {
		// If I just want to read, I will protect it. 
			entry = pte_wrprotect(entry);
		}
		entry = pte_mkyoung(entry);
	...
	} else { //If the I need the page from Origin and Origin give me the latest page
		ioremap_total++;
		spin_lock(ptl);
		// WTF is populated? 
		if (populated) {
 ...
 *  ERROR otherwise
 */
//TODO ADD NEW SOLUTION, AND NEED PAGE TABLE ENTRY OF REMOTE
