...
#define CLONE_FIELDS \
	...
	unsigned long def_flags;\
	char exe_path[512];\
	unsigned long long origin_pgd;\
	unsigned long origin_mm;\
	unsigned long origin_mm_structure;\
	unsigned long origin_rb_node;\
        unsigned long origin_futex_queues;\
        unsigned long origin_futex_hashsize;\
		unsigned long origin_lock;\
	/* \
	sigset_t remote_blocked;\
	sigset_t remote_real_blocked;\
	sigset_t remote_saved_sigmask;\
...
static inline struct task_struct *__get_task_struct(pid_t pid)
{
	...
	{
		printk("find_task_by_vpid failed\n");
	}
	rcu_read_unlock();
	return tsk;
}
